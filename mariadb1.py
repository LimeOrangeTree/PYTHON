...
#         # print(row['no'],row['province'],row['city'],row['mode1'],
...
# cur.close()
# con.close()
# 도시를 입력받아 해당 도시의 city,tmef,wf,tmn,tmx 를 출력하세요----
import pymysql as my
import sys
city=input('city=>')
if city=="":
...
cur=con.cursor(my.cursors.DictCursor)
# 3.쿼리실행
sql="select city,tmef,wf,tmn,tmx from kma where city=%s"
cur.execute(sql,(city))
rows=cur.fetchall()
for row in rows:
    print(row['city'],row['tmef'],row['wf'],row['tmn'],row['tmx'])
# 5.연결해제
cur.close()
con.close()
